...
SelectTool::SelectTool(MainImageScene *scene) : Tool(scene), rubberBand(nullptr), resizing(nullptr), dragging(nullptr)
{
...
//	QGraphicsItem* sel = scene->itemAt(pos);
    ...
    if (itemsHere.count() > 0) {
        ...
        if (item->type() > QGraphicsItem::UserType) {
            ...
            qDebug("first item found at location is of type %d; selected? %d; bounds %d, %d %d x %d", item->type(),
                   item->isSelected(), (int)bounds.x(), (int)bounds.y(), (int)bounds.width(), (int)bounds.height());
        }
//		if (item->type() > QGraphicsItem::UserType)
//		{
